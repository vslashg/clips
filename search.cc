...
void Advance(StateVec& prev, double goal, double opt_time) {
  StateVec next;

  while (!prev.empty()) {
    std::unique_ptr<clips::State> cur = std::move(prev.back());
    prev.pop_back();
    for (auto& item : cur->Branches(goal)) {
      if (item->Clips() == goal || item->Win()) {
...
void AdvanceSharded(StateVec& s, double goal, double opt_time) {
  if (s.size() < 240) {
    Advance(s, goal, opt_time);
  } else {
    ...
    ForEachVec(addrs,
               [goal, opt_time](StateVec& v) { Advance(v, goal, opt_time); });
    s.clear();
...
int main() {
  ...
  int stride = 100;
  for (int i = stride; i < 15000; i += stride) {
    AdvanceSharded(pool, i, time_upper_bound);
  ...
  AdvanceSharded(pool, 15000., time_upper_bound);
